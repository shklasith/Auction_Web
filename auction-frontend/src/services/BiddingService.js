...
class BiddingService {
    ...
    async initialize() {
        try {
            this.connection = new signalR.HubConnectionBuilder()
                .withUrl('http://localhost:5000/biddingHub')
    ...
    async placeBid(auctionId, amount, bidderId) {
        const response = await fetch('http://localhost:5000/api/bidding/place-bid', {
            method: 'POST',
            headers: {
    ...
    async getNextMinimumBid(auctionId) {
        const response = await fetch(`http://localhost:5000/api/bidding/${auctionId}/next-minimum-bid`);
        return await response.json();
    }

    async getAuctionBids(auctionId) {
        const response = await fetch(`http://localhost:5000/api/bidding/${auctionId}/bids`);
        return await response.json();
    }
}
